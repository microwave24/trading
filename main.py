...
SHORT_MA_PERIOD = 8
LONG_MA_PERIOD = 30
MIN_SHORT_DELTA = 0.007  # Minimum short delta to consider a buy signal
MAX_SHORT_DELTA = 0.015
MAX_MA_DISTANCE = 0.02  # Maximum distance between short and long MA to consider a buy signal
...
def get_short_deltas(data):
    # Calculate short deltas (first difference)
    short_deltas = data['open'].diff()
    return short_deltas
...
def get_buy_signal(data):
    ...
    for i in range(LONG_MA_PERIOD, len(data)):
        if data['Long Positions'].iloc[i] == 1 and data['Short Deltas'].iloc[i] > MIN_SHORT_DELTA and data['Short Deltas'].iloc[i] < MAX_SHORT_DELTA:
            buy[i] = 1
    return buy
...
# MAIN EXECUTION
def main():
    historical_data = get_historical_data(API_KEY, SECRET, TARGET_SYMBOL, datetime.now() - timedelta(days=3), datetime.now() - timedelta(days=0.5))
    ...
    mpf.plot(
        ...
        addplot=[MAshort, MAlong, long_markers, buy_markers],
        volume=False
    )
